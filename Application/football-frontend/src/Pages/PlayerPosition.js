...
export default function PlayerPosition() {
  const [playerPositionRows, setPlayerPositionRows] = React.useState([]);
  ...
  React.useEffect(() => {
    axios.get("http://localhost:8080/api/playerPosition").then((res) => {
      var i;
      ...
      setPlayerPositionRows(res.data);
    });
  }, []);
  ...
  return (
    ...
    >
      <div style={{ height: 620, width: "100%", marginBottom: "10px" }}>
        <DataGrid
          rows={playerPositionRows}
          columns={playerPositionColumns}
          pageSize={10}
        />
      </div>
    </div>
